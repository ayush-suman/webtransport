...
type session struct {
	...
	bidiAcceptQueue acceptQueue[quic.Stream]
	uniAcceptQueue  acceptQueue[quic.ReceiveStream]
...
func NewSession(sessionID sessionID, qconn http3.StreamCreator, requestStr quic.Stream) Session {
	...
	c := &session{
		...
		bidiAcceptQueue: *newAcceptQueue[quic.Stream](),
		uniAcceptQueue:  *newAcceptQueue[quic.ReceiveStream](),
...
func (s *session) addStream(qstr quic.Stream, addStreamHeader bool) quic.Stream {
	var hdr []byte
	if addStreamHeader {
		hdr = s.streamHdr
	}
...
func (s *session) addReceiveStream(qstr quic.ReceiveStream) quic.ReceiveStream {
...
func (s *session) addSendStream(qstr quic.SendStream) quic.SendStream {
...
func (s *session) OpenUniStream() (quic.SendStream, error) {
	s.closeMx.Lock()
	defer s.closeMx.Unlock()
...
func (s *session) OpenUniStreamSync(ctx context.Context) (str quic.SendStream, err error) {
	s.closeMx.Lock()
	if s.closeErr != nil {
		s.closeMx.Unlock()
		return nil, s.closeErr
