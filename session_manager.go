...
type SessionManager interface {
	Upgrade(w http.ResponseWriter, r *http.Request) (Session, error)
	AddStream(qconn http3.StreamCreator, str quic.Stream, id sessionID)
	AddUniStream(qconn http3.StreamCreator, str quic.ReceiveStream)
	AddSession(qconn http3.StreamCreator, id sessionID, requestStr quic.Stream) Session
	Close()
}
...
type sessionManager struct {
	...
	mx    sync.Mutex
	conns map[http3.StreamCreator]map[sessionID]*sessionCounter
}

func NewSessionManager(timeout time.Duration) SessionManager {
	m := &sessionManager{
		timeout: timeout,
		conns:   make(map[http3.StreamCreator]map[sessionID]*sessionCounter),
	}
...
func (m *sessionManager) getOrCreateSession(qconn http3.StreamCreator, id sessionID) (sess *sessionCounter, existed bool) {
	m.mx.Lock()
	defer m.mx.Unlock()

	sessions, ok := m.conns[qconn]
	if !ok {
		sessions = make(map[sessionID]*sessionCounter)
	...
	if !ok {
		sess = &sessionCounter{created: make(chan struct{})}
		sessions[id] = sess
	}
	sess.counter++
	return sess, false
...
func (m *sessionManager) handleStream(str quic.Stream, sess *sessionCounter) {
...
func (m *sessionManager) handleUniStream(str quic.ReceiveStream, sess *sessionCounter) {
	t := time.NewTimer(m.timeout)
	defer t.Stop()
...
func (m *sessionManager) AddSession(qconn http3.StreamCreator, id sessionID, requestStr quic.Stream) Session {
	conn := NewSession(id, qconn, requestStr).(*session)
	...
	c := make(chan struct{})
	close(c)
	sessions[id] = &sessionCounter{created: c, conn: conn}
	return conn
}
